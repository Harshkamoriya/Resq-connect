  ...
  useEffect(() => {
    ...
    try {
      ...
      if (helpersStr) {
        ...
        // Any processing with helpers if needed
      }
    ...
    <div className="container mx-auto px-4 py-6 max-w-6xl">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">
          {otpStep ? "Verify your booking" : "Book a roadside assistance professional"}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1">
          <Card className="mb-6">
            <CardHeader>
              <CardTitle>{!otpStep ? "Request Assistance" : "Verify Your Booking"}</CardTitle>
              <CardDescription>
            ...
            <CardContent>
              {!otpStep ? (
                <div className="space-y-4">
                  <div>
                    <label htmlFor="issue-type" className="block text-sm font-medium mb-1">
                      Issue Type
                    </label>
                  ...
                  <div>
                    ...
                    {userData?.name && (
                      <p className="text-xs text-muted-foreground mt-1">Location detected automatically</p>
                    )}
                  </div>
                </div>
              ) : (
            ...
            <CardFooter>
              {!otpStep ? (
                <Button className="w-full" onClick={handleBook} disabled={loading || !issue || !issueType}>
                  {loading ? "Processing..." : "Request Assistance"}
                </Button>
              ) : (
                <Button className="w-full" onClick={verifyOtp} disabled={loading || otp.length !== 4} variant="default">
            ...
            <CardContent>
              <div className="space-y-2">
                ...
                <div className="flex items-center">
                  ...
                  <span className="text-sm">Customer Support: 1-800-ROAD-HELP</span>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="lg:col-span-2 space-y-6">
              ...
              <CardHeader className="pb-3">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <CardDescription>Professional road assistance</CardDescription>
                  </div>
                  <Badge variant={helperInfo.isAvailable ? "success" : "secondary"}>
                ...
                <Tabs defaultValue="details" value={activeTab} onValueChange={setActiveTab}>
                  ...
                  <TabsContent value="details">
                    <div className="flex items-start space-x-4">
                      <Avatar className="h-16 w-16 border">
                        <AvatarImage
                          src={helperInfo.profileImage || "/placeholder.svg?height=64&width=64"}
                          alt={helperInfo.name}
                        />
                        <AvatarFallback>{helperInfo.name?.charAt(0) || "H"}</AvatarFallback>
                      </Avatar>

                      <div className="flex-1">
                        <h3 className="text-xl font-semibold">{helperInfo.name}</h3>

                        <div className="flex items-center mt-1">
                          <Star className="h-4 w-4 text-yellow-500 mr-1" />
                          <span className="font-medium">{helperInfo.ratings || "4.8"}</span>
                          <span className="text-muted-foreground text-sm ml-1">
                            ({helperInfo.completedJobs?.length || "24"} jobs completed)
                          </span>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-4">
                          <div className="flex items-center">
                            ...
                            <span className="text-sm capitalize">{helperInfo.serviceType || "Mechanic"}</span>
                          </div>

                          <div className="flex items-center">
                            <MapPin className="h-4 w-4 mr-2 text-muted-foreground" />
                            <span className="text-sm">{helperInfo.city || "Downtown"}</span>
                          </div>

                          <div className="flex items-center">
                            <Clock className="h-4 w-4 mr-2 text-muted-foreground" />
                            <span className="text-sm">Response time: ~{Math.floor(Math.random() * 15) + 5} min</span>
                          </div>

                          <div className="flex items-center">
                            <Phone className="h-4 w-4 mr-2 text-muted-foreground" />
                            <span className="text-sm">{helperInfo.phone || "Contact via app"}</span>
                          </div>
                        </div>
                        ...
                        <div className="mt-4 pt-4 border-t">
                          ...
                          <div className="flex items-center space-x-3">
                            <Badge variant={helperInfo.availability ? "success" : "secondary"}>
                              {helperInfo.availability ? "Available Now" : "Busy"}
                            </Badge>

                            <div className="flex items-center">
                              <span className="text-sm mr-2">Total Earnings:</span>
                              <Badge variant="outline">${helperInfo.totalEarnings?.toLocaleString() || "2,450"}</Badge>
                            </div>
                          </div>
                        </div>
                      </div>
                  ...
                  <TabsContent value="stats">
                    <div className="space-y-4">
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <Card className="border">
                          <CardHeader className="py-3 pb-1">
                            <CardTitle className="text-base text-center">Completed Jobs</CardTitle>
                          </CardHeader>
                          <CardContent className="py-2 text-center">
                            <p className="text-3xl font-bold text-primary">{helperInfo.completedJobs?.length || 0}</p>
                            <p className="text-xs text-muted-foreground">Total services provided</p>
                          </CardContent>
                        </Card>

                        <Card className="border">
                          <CardHeader className="py-3 pb-1">
                            <CardTitle className="text-base text-center">Rating</CardTitle>
                          </CardHeader>
                          <CardContent className="py-2 text-center">
                        ...
                        <Card className="border">
                          <CardHeader className="py-3 pb-1">
                            <CardTitle className="text-base text-center">Experience</CardTitle>
                          </CardHeader>
                          <CardContent className="py-2 text-center">
                      ...
                      <div>
                        <h3 className="text-sm font-medium mb-2">Service History</h3>
                        <div className="space-y-2">
                          {helperInfo.completedJobs && helperInfo.completedJobs.length > 0 ? (
                            helperInfo.completedJobs.slice(0, 3).map((job, index) => (
                              <div key={index} className="flex items-center justify-between border rounded-md p-3">
                                <div className="flex items-center">
                                  {job.serviceType === "mechanic" && <Tool className="h-4 w-4 mr-2 text-primary" />}
                                  {job.serviceType === "fuel" && <Fuel className="h-4 w-4 mr-2 text-primary" />}
                                  {job.serviceType === "tow" && <Truck className="h-4 w-4 mr-2 text-primary" />}
                                  <div>
                                    <p className="text-sm font-medium capitalize">{job.serviceType} Service</p>
                            ...
                            return (
                              <div key={type} className="space-y-1">
                                <div className="flex items-center justify-between">
                                  <div className="flex items-center">
                                    {type === "mechanic" && <Tool className="h-4 w-4 mr-2" />}
                                    {type === "fuel" && <Fuel className="h-4 w-4 mr-2" />}
                                    {type === "tow" && <Truck className="h-4 w-4 mr-2" />}
                        ...
                        helperInfo.reviews.slice(0, 3).map((review, index) => (
                          <div key={index} className="border-b pb-3 last:border-0">
                            <div className="flex items-center mb-2">
                              ...
                              <div>
                                <p className="font-medium text-sm">{review.user || "Anonymous User"}</p>
                                <div className="flex items-center">
                                  {Array(5)
                                    .fill(0)
                                    .map((_, i) => (
                                      <Star
                                        key={i}
                                        className={`h-3 w-3 ${i < (review.rating || 5) ? "text-yellow-500" : "text-gray-300"}`}
                      ...
                      <Card className="border">
                        <CardHeader className="py-3">
                          <div className="flex items-center">
                            ...
                            <CardTitle className="text-base">Mechanical Repairs</CardTitle>
                          </div>
                        </CardHeader>
                      ...
                      <Card className="border">
                        <CardHeader className="py-3">
                          <div className="flex items-center">
                            ...
                            <CardTitle className="text-base">Fuel Delivery</CardTitle>
                          </div>
                        </CardHeader>
                        <CardContent className="py-2">
                          <p className="text-sm text-muted-foreground">Emergency fuel when you're stranded</p>
                        </CardContent>
                      </Card>
                      ...
                      <Card className="border">
                        <CardHeader className="py-3">
                          <div className="flex items-center">
                            ...
                            <CardTitle className="text-base">Towing Service</CardTitle>
                          </div>
                        </CardHeader>
                        <CardContent className="py-2">
                          <p className="text-sm text-muted-foreground">Towing to nearest service center</p>
                        </CardContent>
                      </Card>
                      ...
                      <Card className="border">
                        <CardHeader className="py-3">
                          <div className="flex items-center">
                            ...
                            <CardTitle className="text-base">Tire Change</CardTitle>
                          </div>
                        </CardHeader>
                        <CardContent className="py-2">
                          <p className="text-sm text-muted-foreground">Quick tire replacement or repair</p>
                        </CardContent>
                      </Card>
                    </div>
                  </TabsContent>
          ...
          <Card>
            <CardHeader className="pb-3">
              ...
              <CardDescription>Your location and nearby helpers</CardDescription>
            </CardHeader>
